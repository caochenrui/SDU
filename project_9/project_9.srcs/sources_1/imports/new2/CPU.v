...
module CPU(
    ...
    //MUX ALU1
    // assign alu1=ctrl[11]?(pc):(readdata1);
    assign alu1=aa;
    //PC
    ...
    //IM
    // assign dout_im=im[addr];
    // assign irf=im[(pc-'h00003000)>>2];
    // always @(posedge clk_ld,negedge rstn) begin:IM
    //     integer i;
    //     if(!rstn) for (i=0;i<64;i=i+1) im[i]<=0;
    //     else if(we_im) im[addr]<=din;
    // end
    dist_mem_gen_0(
    ...
    //DM
    // assign readdata=(ctrl[9])?dm[aluresult>>2]:0;
    // assign dout_dm=dm[addr];
    // always @(posedge clk_ld,negedge rstn) begin:DM
    //     integer i;
    //     if(!rstn) for (i=0;i<64;i=i+1) dm[i]<=0;
    //     else if(we_dm) dm[addr]<=din;
    //     else if(ctrl[3]) dm[aluresult>>2]<=readdata2;
    // end
    wire [31:0]dpo1,spo1;
    dist_mem_gen_1(
