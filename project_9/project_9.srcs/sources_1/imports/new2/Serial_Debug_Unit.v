module Serial_Debug_Unit#(
    ...
    output [5:0]check,
    output clk_cpu,
    input [WIDTH-1:0] npc,pc,ctrl,ir,reg_a,reg_b,imm,branch_y,md,reg_din,ALU_result,pc_chk,
    ...
    // scan(.din_rx(din_rx),.flag_rx(flag_rx),.type_rx(type_rx),.req_rx(req_rx),.busy_rx(busy_rx),.stop(scan_stop));

    // prin(.dout_tx(dout_tx),.type_tx(type_tx),.req_tx(req_tx),.busy_tx(busy_tx));
    ...
    PRDI(.clk(clk_tx),.rstn(rstn),.exe(exe_prdi),
    .ctrl(ctrl),.npc(npc),.pc(pc),.addr_out(addr_prdi),
    .mode(sl_prdi),
    ...
    .ir(ir),.reg_a(reg_a),.reg_b(reg_b),.imm(imm),.branch_y(branch_y),.md(md),.reg_din(reg_din),.ALU_result(ALU_result)
    );
    
    FSMLOAD(.busy(busy_lid),.exe(exe_lid),.type(sl_lid),.addr(addr_lid),.clk(clk_tx),.rstn(rstn),.din(din),.we_dm(we_dm),
    ...
    cpu_control_unit(.pc_chk(pc_chk),.clk(clk_tx),.rstn(rstn),.busy(busy_tbgh),.exe(exe_tbgh),.sel(sl_tbgh),.we(we_tbgh),.bp_in(wd),.halt(halt),
    .clk_cpu(clk_cpu),.dout_tx(dout_tbgh),.type_tx(type_tbgh),.req_tx(req_tbgh),.busy_tx(busy_tx)
    );
    
   always@(posedge(clk_tx),negedge(rstn))
    begin
    if(!rstn)
        begin
        cs<=LOADINST;
    ...
    always @(posedge clk_tx,negedge rstn) begin
        if(!rstn)begin
            check1<=0;
        end
        else begin
