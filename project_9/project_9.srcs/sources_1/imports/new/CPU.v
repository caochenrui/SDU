...
module CPU(
    ...
    output reg [31:0]pc,pce,ir,a,b,pcd,ire,imm,y,yw,irw,irm,mdw,mdr,
    ...
    reg [32:1]ctrd;//jalr,jal,alu1,branch,memread,mentoreg,[4:1]aluop,memwrite,alu2,regwrite
    reg [31:0]immd,aluresult,rd,rdm,rdw,rs1,rs2,bb,aa/*,pcm,pcw*/;
    wire [31:0]irf,alu2,alu1,readdata1,readdata2,writedata,readdata,addsum;
    reg [31:0]rf[0:31];
    assign pc_chk=pce;  

    //MUX PC
    assign addsum=pce+(ctr[13]?4:imm);
    assign npc=ctr[13]?(aluresult&~1):((zero&ctr[10]|ctr[12])?addsum:(pc+4));
    //MUX WD
    assign writedata=ctrw[8]?mdr:yw;
    //MUX ALU2
    assign alu2=ctr[2]?imm:bb;
    //MUX ALU1
    assign alu1=ctr[11]?pce:aa;
    // assign alu1=aa;
    //PC
    ...
    //EX/MEM
    always @(posedge clk_cpu,negedge rstn) begin
        if(!rstn) begin ctrm<=0;y<=0;mdw<=0;irm<=0;rdm<=0;/*pcm<=0*/; end
        else begin ctrm<=ctr;y<=ctr[13]?addsum:aluresult;mdw<=bb;irm<=ire;rdm<=rd;/*pcm<=pce*/; end
    end
    //MEM/WB
    always @(posedge clk_cpu,negedge rstn) begin
        if(!rstn) begin ctrw<=0;mdr<=0;yw<=0;irw<=0;rdw<=0;/*pcw<=0*/; end
        else begin ctrw<=ctrm;mdr<=readdata;yw<=y;irw<=irm;rdw<=rdm;/*pcw<=pcm*/; end
    end
    //IM
    dist_mem_gen_0(
    ...
    always @(*) begin
        case (ir[6:0])
            ...
            'b0010011: case (ir[14:12])
                            ...
                            default: begin ctrd=0;type=2; end
                        endcase
            'b1100111: begin ctrd='b100000_0010_011;type=2; end//jalr
            ...
            'b1101111: begin ctrd='b010000_1011_001;type=5; end//jal
    ...
    always @(*) begin
        case (ctr[7:4])
            ...
            11:aluresult=alu1+4;//1011
            12:aluresult=~(alu1|alu2);//1100
            // 13:aluresult=alu1+4;//1101
            default: aluresult=0;
        endcase
    end
    ...
    always @(*) begin//B
        if(ctr[12]|(zero&ctr[10])) begin dflush=1; end
        else begin dflush=0;end
    end
    always @(*) begin
        if(ctr[12]|(zero&ctr[10])|(rd==ir[19:15]|rd==ir[24:20])&ctr[9]) begin eflush=1; end
