module FSMLOAD(
    ...
    always @(posedge clk or negedge rstn)
        if (!rstn) begin busy<=0;req_rx<=0;we_dm<=0;we_im<=0;addr<=-1;din<=0;count<=10;dout_tx<=0;req_tx<=0; end
        else begin
            case (ns)
                ZERO:   
                    begin 
                        busy<=0;req_rx<=0;we_dm<=0;we_im<=0;addr<=-1;din<=0;count<=10;dout_tx<=0;req_tx<=0; 
                    end
                FOURTEEN:
                    begin
                        we_im<=0;
