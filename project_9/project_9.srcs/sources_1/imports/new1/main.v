...
Serial_Debug_Unit(.pc_chk(pc_chk),.scan_state(scan_state),.busy(busy),.check1(check1),.check(check),
...
.addr(addr),.dout_rf(dout_rf),.dout_dm(dout_dm),.dout_im(dout_im),.din(din),.we_dm(we_dm),.we_im(we_im),
// .ld(ld),
.npc(npc),.pc(pc),.ir(ir),.pcd(pcd),
.ire(ire),.imm(imm),.a(a),.b(b),.pce(pce),.ctr(ctr),
.irm(irm),.mdw(mdw),.y(y),.ctrm(ctrm),
